...
export function RankedLobbyClient({ user, initialQueueEntry, recentGames }: Props) {
  ...
  const handleJoinQueue = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(err.error ?? 'Chyba pri vstupu do fronty')
        return
      }
  ...
  const handleLeaveQueue = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error('Chyba pri odchodu z fronty')
        return
      }
  ...
  return (
    <div className="relative min-h-screen paper-bg">
      ...
      <div className="relative container mx-auto px-4 py-10 max-w-3xl">
        <h1 className="text-4xl font-bold tracking-tight mb-2">Ranked mod</h1>
        <p className="text-muted-foreground mb-10 text-sm">
          Soutezni hry s ELO systemem. Hraci jsou parovani dle podobneho ratingu.
        </p>

        {/* ELO card */}
        <div className="card-clubhouse rounded-2xl p-6 mb-6 flex items-center justify-between">
          <div>
            <p className="font-semibold text-lg">{user.nickname}</p>
            <p className="text-muted-foreground text-xs mt-0.5">Vas aktualni rating</p>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/8">
            <svg className="w-4 h-4 text-[#8B6914]" fill="currentColor" viewBox="0 0 20 20">
        ...
        {/* Matchmaking card */}
        <div className="card-clubhouse rounded-2xl p-8 mb-8 text-center">
          <h2 className="font-bold text-lg mb-6">Matchmaking</h2>
          ...
          {inQueue ? (
            <div className="space-y-6">
              ...
              <p className="text-lg font-semibold">Hledam soupere...</p>
              <p className="text-4xl font-mono font-bold text-primary">
                {formatTime(waitTime)}
              </p>
              <p className="text-sm text-muted-foreground">
                Hledam hrace s ELO v rozsahu +/-{eloRange}
              </p>
              ...
              >
                {loading ? 'Odchazim...' : 'Opustit frontu'}
              </Button>
            </div>
          ) : (
            <div className="space-y-5">
              <div className="inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-primary/8 mx-auto">
              ...
              <p className="text-muted-foreground text-sm max-w-sm mx-auto">
                Pripraveni na ranked hru? System vam najde soupere s podobnym ELO.
              </p>
              <Button size="lg" onClick={handleJoinQueue} disabled={loading} className="glow-primary px-10">
                {loading ? 'Vstupuji...' : 'Hledat soupere'}
              </Button>
            </div>
          )}
        </div>
        ...
        {recentGames.length > 0 && (
          <div className="card-clubhouse rounded-2xl overflow-hidden">
            <div className="px-6 py-4 border-b">
              <h2 className="font-bold text-base">Nedavne ranked hry</h2>
            </div>
            <div className="divide-y divide-border">
              {recentGames.map((game) => {
                ...
                return (
                  <div
                    key={game.id}
                    className="flex items-center justify-between px-6 py-4 cursor-pointer hover:bg-secondary transition-colors"
                    onClick={() => router.push(`/hry/${game.id}`)}
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                        !isFinished ? 'bg-primary/8' :
                        isWinner ? 'bg-[#8B6914]/10' : 'bg-destructive/8'
                      }`}>
                        {!isFinished ? (
                      ...
                      <div>
                        <p className="text-sm font-medium">
                          {!isFinished ? 'Probiha' : isWinner ? 'Vyhra' : 'Prohra'}
                        </p>
                    ...
                    {myPlayer && (
                      <span className="font-mono text-sm px-3 py-1 rounded-lg bg-primary/8 text-primary">
                        {myPlayer.score} b
                      </span>
                    )}
                  </div>
